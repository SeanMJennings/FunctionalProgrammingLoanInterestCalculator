...
public static partial class MoneySpecsShould
{
    ...
    {
        Given(a_positive_decimal_value).When(converting_to_money).Then(money_is_formatted_to_2_dp);
    }
        
    [Test]
    public static void not_allow_negative_amount()
    {
        Given(a_negative_decimal_value).WhenValidating(converting_to_money).ThenInforms("Amount cannot be negative.");
    }
